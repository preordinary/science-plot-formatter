...
Use the `Read` tool on the produced PNG to actually see the figure. Do not skip this — you must look at the pixels.
...
- **Legend placement — default is INSIDE the axes, `ncol=1`.** Keep whatever `loc=` the user picked (usually `"upper left"`, `"best"`, etc.) and tighten the legend via pure-shrink levers — smaller `legend.fontsize`, shorter `legend.handlelength`, tighter `legend.labelspacing` / `legend.borderpad` / `legend.handletextpad`. **Do not reach for `ncol=2` as a routine tightening step.** `ncol=2` is not a size reduction — it is a shape trade that converts vertical footprint into horizontal footprint. On a column-width figure (~3.25" wide), a 4-entry legend at `ncol=1` + 5.5pt font occupies roughly 0.8" wide × 0.6" tall; the same legend at `ncol=2` becomes ~1.6" wide × 0.3" tall, a 0.8" width addition for a 0.3" height saving — almost always a losing trade because the added horizontal footprint sweeps past any existing empty quadrant into the data. Only use `ncol>1` when *all* of: (a) font has already been reduced to body_pt−4 or smaller *and* (b) `ncol=1` vertical height still exceeds ~40% of the axes height *and* (c) the axes is genuinely wider than tall and has empty horizontal space where the added width will land. On short column-width panels, the right answer when the legend feels too tall is almost always "smaller font, stay at ncol=1," not "ncol=2."
- **Moving the legend outside the axes is a last resort.** Using `bbox_to_anchor=(0.5, -0.3)`, `loc="center left", bbox_to_anchor=(1.02, 0.5)`, etc. is a structural layout change — it shifts margins, competes with twin-axis labels, and permanently consumes figure real estate. Only move the legend outside when *all* of: (a) the legend still covers ≥ ~40% of the data region after in-axes shrinking, *and* (b) reducing `legend.fontsize` further would drop it below ~5pt on the printed page, *and* (c) there is no empty quadrant of the plot the legend could relocate to with `loc=` alone. Same reasoning for the on-figure title — don't convert `ax.set_title(...)` into a `fig.suptitle(...)` or an above-axes anchored legend as a space-saving shortcut.
- **Legend position awareness — don't just keep the user's `loc=`, verify it doesn't cover data.** In the baseline render, identify where the *data climax* sits (the region with tallest bars, the line's peak, the densest marker cluster, the right-edge of a monotonic growth curve). The legend must not land there after your visual tightening, because a seemingly-safe `loc="upper left"` becomes unsafe once `ncol=2` widens the legend box to span most of the axis width, and likewise `loc="best"` can pick different quadrants across re-renders. Procedure: (1) from the baseline, name the busiest quadrant of the axes (e.g. "upper right — cumulative line spikes to 2.2 at the 15→16 transition"); (2) name the emptiest quadrant (e.g. "lower center — bars and lines are all in the lower decade for transitions 0–12"); (3) place the legend in the emptiest region, using `loc=` alone when possible. Heuristics that handle most cases: monotonically increasing series → legend in *upper left* only if it is narrower than the plot's left half (ncol=1), else *lower right* below the rise; decreasing series → *upper right*; symmetric distributions/heatmaps → *upper right* or outside; scatter clusters → the corner farthest from the centroid. Default to `ncol=1` (vertical stack) — it is the narrower footprint and fits most empty quadrants. If the legend at `ncol=1` still overflows an empty quadrant, shrink `legend.fontsize` first (down to body_pt−4 or −5, typically 5.5–6pt). `ncol=2` is a *width trade* that spreads the legend horizontally and should not be used unless `ncol=1` at minimum readable font still does not fit — and even then, only after you have checked what sits under the doubled horizontal footprint.
...
1. **Measure edge padding in pixels — do not rely on visual judgement.** Visual inspection of a rendered thumbnail misses 1–3 pixel clipping of rotated text, long twin-axis labels, and descenders.
2. Walk through each item in the Phase 3 issue list and mark it **resolved**, **partial**, or **still present**, with a one-line justification tied to what you see in the new image.
3. **Scan for regressions the edits could have introduced:**
   ...
   - **Legend now overlapping the data climax.** Even though the legend is inside the axes, did `ncol=2` widen its horizontal footprint over the data's peak (e.g. a cumulative line's right-edge spike, the tallest bar)? Did the smaller figsize change which quadrant is "empty"? Re-check the legend's position against the busiest region named in Phase 4. If the legend's new horizontal extent covers the climax, drop to `ncol=1` (vertical stack, narrower footprint) with smaller `legend.fontsize`, or change `loc=` to the actually-empty quadrant, before considering an outside-axes move.
4. **Common failures and fixes:**
   ...
   - *Legend dominates data region on small figsize* — **keep it inside the axes at `ncol=1`**, shrink it visually. Apply in strict order: (1) drop `legend.fontsize` to body_pt−4 or body_pt−5 (typically 5.5–6pt at column width — still readable on the page), (2) shrink `legend.handlelength` to 0.8, `legend.labelspacing` to 0.2, `legend.borderpad` to 0.2, `legend.handletextpad` to 0.25, (3) try a different in-axes `loc=` if the legend's vertical footprint now fits better in another corner. **Only if vertical height still exceeds ~40% of the axes** after steps 1–3 should `ncol=2` be considered — and only then after verifying the doubled horizontal footprint won't cover data elsewhere. Never reach for `ncol=2` before font has been shrunk. Outside-axes (`bbox_to_anchor=(0.5, -0.3)` etc.) is the final last resort, used only when font would otherwise fall below ~5pt on the printed page. A readable-but-tiny `ncol=1` in-axes legend is almost always the right answer.
5. If any Phase 3 issue is still present, or a regression was introduced, go back to Phase 4 with a targeted adjustment, edit, and re-render. Cap at 2–3 iterations before checking in with the user — do not loop silently.
